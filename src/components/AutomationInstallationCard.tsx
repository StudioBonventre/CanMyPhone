...
export function AutomationInstallationCard({ automation, onHandoff, onConfirm, onCancel }:{ automation:StoredAutomation; onHandoff:()=>void; onConfirm:()=>void; onCancel:()=>void }) {
  ...
  return <ContentSurface emphasis="active" style={styles.card}>
    ...
    {setup ? <View style={styles.steps}>{setup.setupSteps.map((step,index)=><View key={`${index}-${step}`} style={styles.step}><Text style={styles.number}>{index+1}</Text><Text style={styles.stepText}>{step}</Text></View>)}</View> : null}
